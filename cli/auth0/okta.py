...
from auth0.constants import (
    OKTA_DOMAIN, OKTA_CLIENT_ID, CODE_CHALLENGE_METHOD, SCOPE, OKTA_REDIRECT_URI, IDP
)
...
def create_crypto_pair() -> CRYPTOPAIR:
    ...
    """
    try:
        verifier = base_64_urlencode(secrets.token_bytes(64))
...
def authorize_user(challenge_str) -> None:
    ...
    try:
        url = OKTA_DOMAIN + urllib.parse.urlencode(params)
        return url
    except TypeError as tye:
        print(tye)
        return None
...
def exchange_code_for_token(code: str, verifier: bytes) -> str:
    ...
    res = requests.post('https://dev-681731.oktapreview.com/oauth2/default/v1/token', json=payload)
